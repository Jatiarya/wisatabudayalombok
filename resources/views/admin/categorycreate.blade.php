<x-admin-layout title="Tambah Kategori">
    ...
    <div class="glass-panel p-6 max-w-lg">
        <h2 class="font-bold text-lombok-earth mb-1">Tambah Kategori</h2>
        <p class="text-sm text-lombok-earth/60 mb-5">Buat kategori baru untuk mengelompokkan destinasi wisata.</p>

        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-4">
            @csrf
            <div>
                <label class="text-sm font-medium text-lombok-earth">Nama Kategori</label>
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="contoh: Pantai"
                    class="input-glass w-full mt-1">
            </div>

            <div x-data="{ icon: '{{ old('icon') }}' }" x-init="$nextTick(() => lucide.createIcons())">
                ...
                <div class="flex items-center gap-3 mt-1">
                    <div x-ref="iconPreview"
                        ...
                        <i data-lucide="{{ old('icon') ?: 'image' }}" class="h-5 w-5"></i>
                    </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="btn-glow"> <i data-lucide="save" class="h-4 w-4"></i>Simpan</button>
