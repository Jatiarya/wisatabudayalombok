<x-admin-layout title="Edit Kategori Produk">
    ...
    <div class="glass-panel p-6 max-w-lg">
        <h2 class="font-bold text-lombok-earth mb-1">Edit Kategori Produk</h2>
        <p class="text-sm text-lombok-earth/60 mb-5">Perbarui informasi kategori <span
                class="font-medium">{{ $productcategory->name }}</span>.</p>

        <form method="POST" action="{{ route('admin.productcategories.update', $productcategory) }}" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="input-glass w-full mt-1">{{ old('description', $productcategory->description) }}</textarea>
            </div>
