<x-admin-layout title="Kategori Produk">
    <div x-data x-init="$nextTick(() => lucide.createIcons())">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
            <p class="text-sm text-lombok-earth/60">Kelola kategori produk khas Lombok.</p>
            <a href="{{ route('admin.productcategories.create') }}" class="btn-glow">
                        ...
                        <tr class="border-t border-white/40 hover:bg-white/20 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex gap-2 justify-end">
                                    ...
                                    <form id="delete-product-category-{{ $category->id }}" method="POST"
                                        action="{{ route('admin.productcategories.destroy', $category) }}">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                    <button type="button" title="Hapus kategori produk"
                                        @click="$dispatch('confirm-delete', { formId: 'delete-product-category-{{ $category->id }}', label: '{{ $category->name }}' })"
        ...
        <div class="mt-4">{{ $productCategories->links() }}</div>
    </div>
</x-admin-layout>
